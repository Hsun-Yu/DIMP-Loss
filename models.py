...
class MyLoss(nn.Module):
    def __init__(self, gamma=1, alpha=None, size_average=True, mode="dimp"):
        """
        mode="dimp": loss = -alpha * (pt2/pt)**gamma * log(pt)
        mode="imp":  loss = -alpha * (pt2/pt3)**gamma * log(pt)
        """
...
class MyNewBertForSequenceClassification(BertPreTrainedModel):
    ...
    def forward(
        ...
        if labels is not None:
            ...
            elif self.config.problem_type == "single_label_classification_dimp":
                loss_fct = MyLoss(mode="dimp")
                loss = loss_fct(logits.view(-1, self.num_labels), labels.view(-1), outputs2.logits.view(-1, self.num_labels))
            elif self.config.problem_type == "single_label_classification_imp":
                loss_fct = MyLoss(mode="imp")
